...
use agent_core::{
    AgentConfig, Activity, ActivityRequest, ConfigLoader, ImageFormat, RegisterRequest,
    Screenshot, ScreenshotRequest, SystemInfo, VERSION,
    HeartbeatRequest,
};
use anyhow::Result;
...
impl Agent {
    ...
    async fn run(&self, running: Arc<AtomicBool>) -> Result<()> {
        ...
        let idle_threshold = self.config.thresholds.idle_secs;
        ...
        let screenshot_config = self.config.screenshot.clone();
        let sys_info = SystemInfo::current();
