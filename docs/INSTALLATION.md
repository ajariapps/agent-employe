...
cd employee-monitoring/agent-rust
cargo build --release
...
```bash
# 1. Download dan extract release
curl -L https://github.com/yourorg/employee-monitoring/releases/latest/download/agent-rust-macos-x86_64.tar.gz -o agent-rust.tar.gz
tar xzf agent-rust.tar.gz
cd agent-rust-macos-x86_64
...
sudo cp scripts/com.yourorg.agent-rust.plist /Library/LaunchDaemons/
sudo launchctl load /Library/LaunchDaemons/com.yourorg.agent-rust.plist
```
...
Invoke-WebRequest -Uri "https://github.com/yourorg/employee-monitoring/releases/latest/download/agent-rust-windows-x64.zip" -OutFile "agent-rust.zip"
Expand-Archive -Path agent-rust.zip -DestinationPath .
cd agent-rust-windows-x64
...
```bash
# Start service
sudo launchctl start com.yourorg.agent-rust

# Stop service
sudo launchctl stop com.yourorg.agent-rust

# Restart service
sudo launchctl kickstart -k gui/$(id -u)/com.yourorg.agent-rust
...
sudo launchctl stop com.yourorg.agent-rust
sudo launchctl unload /Library/LaunchDaemons/com.yourorg.agent-rust.plist

# Remove files
sudo rm /Library/LaunchDaemons/com.yourorg.agent-rust.plist
sudo rm /usr/local/bin/agent
sudo rm -rf /etc/agent-rust
```
